...
use crate::utils::{create_shortcut, exe_has_signature, get_exe_description, get_native_arch, get_program_arch, get_shortcut_target, has_icon_in_program, is_gui_program, is_running_under_wow64, launched_from_explorer, matches_glob, normalize_app_name, parse_icon_spec, replace_ignore_case, resolve_relative_path, validate_shortcut_name_for_config};
use clap::Parser;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if cli.config.is_some() && cli.targetPath.is_none() && cli.lnkPath.is_none() {
        ...
        config_shortcut(cfg, cli.install, cli.install_parallel, cli.start, cli.use_filename)?;
        return Ok(());
    }

    // 自动搜索模式
    auto_shortcut(&cli.targetPath.unwrap(), cli.lnkPath.as_deref(), cli.config.as_deref(), cli.only_match, cli.create_dir, cli.install, cli.install_parallel, cli.start, cli.list, cli.use_filename, cli.score_ratio)?;
    Ok(())
}
...
pub fn auto_shortcut(
    ...
    if let Some(config) = config_path {
        if let Ok(mut config) = ConfigInfo::parse_config_file(config) {
            ...
            if let Some(ratio) = config.score_ratio {
                if ratio > 1.0 {
                    write_console(ConsoleType::Warning, &t!("config.invalid_ratio", ratio = ratio));
            ...
            for ln in &config.shortcut {
                if let Some(ref provided_name) = ln.name {
                    if !validate_shortcut_name_for_config(provided_name) {
                        write_console(ConsoleType::Warning, &t!("config.invalid_name", name = provided_name));
                    }
                }
            }
    ...
    // 主循环: 遍历所有文件（包括子目录）
    for entry_result in WalkDir::new(target_path).into_iter()
        .filter_entry({
            ...
            move |entry| {
                ...
                if entry.file_type().is_dir() {
                    if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                        if EXCLUDED_DIRS.iter().any(|&ex| ex.eq_ignore_ascii_case(name)) {
                            if DEBUG.load(Ordering::Relaxed) {
                                write_console(ConsoleType::Debug, &t!("scan.ignore", path = path.display()));
                            }
                            return false;
                        }
                    }
        ...
        if entry.file_type().is_dir() {
            if is_category_dir(file_path) {
                if !list_mode {
                    write_console(ConsoleType::Info, &t!("directory.category", path = file_path.display()));
                }
                continue;
            }
            ...
            if is_single_file_dir(file_path) {
                // 单文件程序目录
                if !list_mode {
                    write_console(ConsoleType::Info, &t!("directory.single_file", path = file_path.display()));
                }

                let mut roots = identified_app_roots.lock().unwrap();
                if roots.insert(file_path.to_path_buf()) {
                    // println!("剪枝: {}", file_path.display());
                    for entry in WalkDir::new(file_path).max_depth(1).into_iter()
                        ...
                        if let Some(cfg) = &config_info {
                            ...
                            if path.file_name().map(|n| n.to_string_lossy().to_lowercase())
                                .is_some_and(|name| cfg.ignore.iter().any(|kw| name.contains(&kw.to_lowercase()))) {
                                if DEBUG.load(Ordering::Relaxed) {
                                    write_console(ConsoleType::Debug, &t!("scan.ignore", path = path.display()));
                                }
                                continue;
                        ...
                        // 运行程序
                        if start {
                            write_console(ConsoleType::Info, &t!("shortcut.start", path = path.display()));
                            Command::new(path).creation_flags(0x08000000).current_dir(path.parent().unwrap()).spawn().ok();

                            // 如果命令行没有指定 lnk_path，且配置里也没有对应的 dest，则跳过后续处理
                            if lnk_path.is_none() && config_info.as_ref()
                                .and_then(|cfg| Lnk::get_lnk_info(path, &cfg.shortcut))
                                .as_ref()
                                .and_then(|li| li.dest.as_ref()).is_none() {
                                continue;
                            }
                        }
                        ...
                        // 创建快捷方式
                        let lnk_info = config_info.as_ref()
                            .and_then(|cfg| Lnk::get_lnk_info(path, &cfg.shortcut));
                        let template = config_info.as_ref().and_then(|cfg| cfg.template.clone());

                        match create_program_shortcut(path, lnk_path, lnk_info, template, use_filename, create_dir) {
                            Ok((name, _path)) => write_console(ConsoleType::Success, &t!("shortcut.create_success", name = name, path = path.display())),
                            Err(_) => write_console(ConsoleType::Error, &t!("shortcut.create_failed", path = path.display()))
                        };
                    }
                } else {
                    if DEBUG.load(Ordering::Relaxed) {
                        write_console(ConsoleType::Debug, &t!("scan.prune", path = file_path.display()));
                    }
                };
            } else if is_hybrid_software_dir(file_path, &all_excluded) {
                // 单文件软件、绿色软件混合目录
                if !list_mode {
                    write_console(ConsoleType::Info, &t!("directory.hybrid", path = file_path.display()));
                }
                continue;
            } else {
                // 绿色软件目录
                if !list_mode {
                    write_console(ConsoleType::Info, &t!("directory.green", path = file_path.display()));
                }

                let mut roots_guard = identified_app_roots.lock().unwrap();

                // 遍历全部exe进行打分
                if let Some((suggested_app_root, exe_path)) = find_software_best_exe(file_path, config_info.clone().as_ref(), target_path, score_ratio, list_mode) {
                    // 找到了最佳EXE，并且它的根目录是新的（没有被处理过）
                    if roots_guard.insert(suggested_app_root.clone()) {
                        // 运行安装脚本
                        if install_script {
                            run_install_scripts(&suggested_app_root, config_info.as_ref().map(|config| config.scripts.clone()).as_deref(), install_parallel);
                        }
                        ...
                        // 运行程序
                        if start {
                            write_console(ConsoleType::Info, &t!("shortcut.start", path = exe_path.display()));
                            Command::new(&exe_path).creation_flags(0x08000000).current_dir(exe_path.parent().unwrap()).spawn().ok();
                        ...
                        match create_program_shortcut(&exe_path, lnk_path, lnk_info, template, use_filename, create_dir) {
                            Ok((name, _path)) => write_console(ConsoleType::Success, &t!("shortcut.create_success", name = name, path = exe_path.display())),
                            Err(_) => write_console(ConsoleType::Error, &t!("shortcut.create_failed", path = exe_path.display()))
                        };
                    } else {
                        // 已处理软件根目录
                        if DEBUG.load(Ordering::Relaxed) {
                            write_console(ConsoleType::Debug, &t!("scan.prune", path = suggested_app_root.display()));
                        }
                    }
                } else {
                    // 绿色软件目录中，根据评分规则没有识别到主程序
                    if !list_mode {
                        write_console(ConsoleType::Warning, &t!("scan.main_not_recognized", path = file_path.display()));
                    }
                    roots_guard.insert(file_path.to_path_buf());
                }
            }
        } else if entry.file_type().is_file() && file_path.extension().is_some_and(|ext| ext.eq_ignore_ascii_case("exe")) {
            ...
            // 运行程序
            if start {
                write_console(ConsoleType::Info, &t!("shortcut.start", path = file_path.display()));
                Command::new(file_path).creation_flags(0x08000000).current_dir(file_path.parent().unwrap()).spawn().ok();
            ...
            match create_program_shortcut(file_path, lnk_path, lnk_info, template, use_filename, create_dir) {
                Ok((name, _path)) => write_console(ConsoleType::Success, &t!("shortcut.create_success", name = name, path = file_path.display())),
                Err(_) => write_console(ConsoleType::Error, &t!("shortcut.create_failed", path = file_path.display()))
            };
        }
    }
...
fn config_shortcut(config_path: PathBuf, install: bool, install_parallel: bool, start: bool, use_name: bool) -> Result<(), Box<dyn Error>> {
    // 读取配置文件信息
    ...
    if install {
        for pat in config_info.scripts.iter() {
            ...
            write_console(ConsoleType::Info, &t!("shortcut.run_install", path = file_path.display()));
            if install_parallel {
    ...
    for lnk in config_info.shortcut {
        // 运行程序
        if start {
            Command::new(&lnk.exec).creation_flags(0x08000000).current_dir(Path::new(&lnk.exec).parent().unwrap()).spawn().ok();
        }

        // 创建快捷方式
        match create_program_shortcut(Path::new(&lnk.exec.clone()), None, Some(lnk.clone()), config_info.template.clone(), use_name, false) {
            Ok((name, _path)) => write_console(ConsoleType::Success, &t!("shortcut.create_success", name = name, path = lnk.exec)),
            Err(_) => write_console(ConsoleType::Error, &t!("shortcut.create_failed", path = lnk.exec)),
        };
    }
...
fn contains_app_structure_lightweight(dir_path: &Path) -> bool {
    if let Ok(entries) = fs::read_dir(dir_path) {
            ...
            if file_type.is_file() {
                if let Some(ext) = path.extension().and_then(|s| s.to_str()) {
                    ...
                    if lower_ext == "exe" {
                        has_exe = true;
                        exe_count += 1;
                    } else if ["dll", "pak", "ini", "json", "xml", "yaml", "dat", "cfg", "conf", "log", "reg", "key", "cupf"].contains(&lower_ext.as_str()) {
                        has_support_files = true;
                    } else if ["txt", "md", "pdf"].contains(&lower_ext.as_str()) {
                        if let Some(file_stem) = path.file_stem().and_then(|s| s.to_str()) {
                            let lower_stem = file_stem.to_ascii_lowercase();
                            if ["readme", "license", "eula", "changelog"].contains(&lower_stem.as_str()) {
                                has_doc_files = true;
                            }
                        }
                    }
                ...
                if let Some(dir_name) = path.file_name().and_then(|s| s.to_str()) {
                    ...
                    if ["bin", "lib", "data", "program", "assets", "resources", "content", "modules", "plugins", "drivers"].contains(&lower_dir_name.as_str()) {
        ...
        let is_component_folder_name = dir_path.file_name().is_some_and(|n| {
            ...
            ["bin", "program", "executables", "x64", "win64", "modules", "plugins", "drivers"].contains(&lower_name.as_str())
        });
...
fn find_software_best_exe(app_root_path: &Path, config_info: Option<&ConfigInfo>, initial_scan_root: &Path, score_ratio: f32, list_mode: bool) -> Option<(PathBuf, PathBuf)> {
    ...
    for entry_result in WalkDir::new(app_root_path).max_depth(2).into_iter() { //
        let entry = match entry_result {
            ...
            Err(e) => {
                ...
                write_console(ConsoleType::Warning, &format!("Access entry failed: {}", e));
                continue;
            }
        };
        ...
        if file_path.is_file() && file_path.extension().is_some_and(|ext| ext.eq_ignore_ascii_case("exe")) {
            /// 最高分
            ...
            // 应用配置中的忽略列表
            if let Some(cfg) = &config_info {
                let file_name = file_path.file_name().map(|n| n.to_string_lossy().to_lowercase());
                ...
                if file_name.is_some_and(|name| {
                    ...
                    if DEBUG.load(Ordering::Relaxed) {
                        write_console(ConsoleType::Debug, &t!("scan.ignore_in_config", path = file_path.display()));
                    }
                    continue;
                }
            }
            ...
            // 文件名与父目录名匹配
            if let Some(parent_dir_name) = file_path.parent().and_then(|p| p.file_name()).and_then(|n| n.to_str()) {
                if let Some(file_stem) = file_path.file_stem().and_then(|s| s.to_str()) {
                    ...
                    if !normalized_parent_name.is_empty() && (normalized_file_stem.contains(&normalized_parent_name) || normalized_parent_name.contains(&normalized_file_stem)) {
                        score += 40;
            ...
            // 判断文件名包含排除功能关键词
            let exclude_keyword = ["setup", "install", "uninst", "uninstall", "unins", "updater", "config", "editor", "server", "diagnostics", "service", "crashhandler", "helper", "卸载"];
            for exclude in exclude_keyword {
                if file_path.file_name().unwrap().to_ascii_lowercase().to_str().unwrap().contains(exclude) {
                    score -= 50;
            ...
            if DEBUG.load(Ordering::Relaxed) {
                let details = breakdown.iter().map(|(k, v)| format!("{}{:+}", k, v)).collect::<Vec<_>>().join(", ");
                write_console(ConsoleType::Debug, &t!("scan.score_calculation", file = file_path.file_name().unwrap().to_string_lossy(), score = score, details = details));
            }
            ...
            // 识别应用程序根目录 (向上回溯)
            let mut current_root_candidate = file_path.parent().map_or_else(|| file_path.to_path_buf(), |p| p.to_path_buf());
            ...
            while let Some(parent) = current_root_candidate.parent() {
                    ...
                    for entry_in_parent in entries.filter_map(|e| e.ok()) {
                        ...
                        if entry_path.is_file() {
                            if let Some(ext) = entry_path.extension().and_then(|s| s.to_str()) {
                                let lower_ext = ext.to_ascii_lowercase();
                                if lower_ext == "dll" || ["ini", "json", "xml", "dat", "cfg", "conf"].contains(&lower_ext.as_str()) {
                            ...
                            if let Some(dir_name) = entry_path.file_name().and_then(|s| s.to_str()) {
                                let lower_dir_name = dir_name.to_ascii_lowercase();
                                if ["bin", "lib", "data", "program", "assets", "resources", "content"].contains(&lower_dir_name.as_str()) {
                                    parent_has_app_structure = true;
                                    break;
                                }
                            }
                ...
                let current_dir_name_lower = current_root_candidate.file_name().map_or("".to_string(), |n| n.to_string_lossy().to_ascii_lowercase());
                let is_component_folder = ["bin", "program", "executables", "x64", "win64"].contains(&current_dir_name_lower.as_str());
        ...
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            if path.is_file() {
                if let Some(ext) = path.extension().and_then(|s| s.to_str()) {
                    ...
                    if lower_ext == "exe" {
                        exe_count += 1;
                    } else if lower_ext != "ico" { // 允许.ico 文件存在
                        other_file_count += 1;
                    }
                } else {
...
fn is_hybrid_software_dir(dir: &Path, exclude_keyword: &[&str]) -> bool {
    ...
    let total_root_files = (root_exe_count + root_allowed_file_count + root_unknown_file_count) as f32;
    ...
    for entry in WalkDir::new(dir).max_depth(1).into_iter()
        .filter_entry({
            move |entry| {
                ...
                // 默认规则
                if file_name.ends_with("setup.cmd") || file_name.ends_with("setup.bat")
                    || file_name.ends_with("install.cmd") || file_name.ends_with("install.bat")
                    || file_name.contains("绿化.cmd") || file_name.contains("绿化.bat") {
                    return true;
                }
...
fn create_program_shortcut(
    ...
    if let Ok(true) = is_running_under_wow64() {
        ...
        if !Path::new(&alt).exists() {
            write_console(ConsoleType::Warning, &t!("file.not_found",path = program_path.display()));
            return Err(t!("file.not_found",path = program_path.display()).into());
        }
    ...
    let dest = &lnk_info.clone()
        ...
        .or_else(|| template.clone().clone().and_then(|t| t.dest.as_ref().map(PathBuf::from)));
    let mut dest = if let Some(dest) = dest {
        dest.to_path_buf()
    } else {
        write_console(ConsoleType::Warning, &t!("config.miss_dest", path = program_path.display()));
        return Err("configuration missing `dest`".into());
    };
    ...
    // 快捷方式名称
    let mut name = {
        let stem = program_path.file_stem().and_then(|s| s.to_str()).unwrap_or_default();
        let name_template = template.as_ref().and_then(|t| t.name.as_deref()).unwrap_or(DEFAULT_NAME_TEMPLATE);
        ...
        if use_filename {
            ...
            if let Some(name) = &link_info.name {
                if validate_shortcut_name_for_config(name) {
                    ...
                    write_console(ConsoleType::Warning, &t!("config.invalid_name", name = name));
                    return Err(t!("config.invalid_name", name = name).into());
                }
            } else {
    ...
    let icon: Option<(String, i32)> = lnk_info
        ...
        .and_then(|raw| {
            ...
            // 如果找不到就警告
            write_console(ConsoleType::Warning, &t!("file.icon_not_found", path = file_part));
            None
        })
        // 如果 link_info.icon 没有，则尝试使用全局模板配置
        .or_else(|| template.as_ref().and_then(|t| {
            t.icon.as_ref().and_then(|s| {
                let rendered = process_template(program_path, s);
                if Path::new(&rendered).exists() {
                    Some((rendered, 0))
                } else {
                    write_console(ConsoleType::Warning, &t!("file.icon_not_found", path = rendered));
                    None
                }
            })
        }))
        ...
        .or_else(|| {
            program_path.parent().and_then(|p| {
                ...
                sibling.exists().then(|| (sibling.to_string_lossy().to_string(), 0))
            })
        });

    // 工作路径
    let work_dir = lnk_info.clone()
        // 首先使用配置项中的信息
        .and_then(|li| li.work_dir.as_ref().map(|s| s.to_string()))
        // 尝试全局模板配置
        .or_else(|| template.clone().and_then(|t| t.work_dir.as_ref().map(|s| s.to_string())));

    // 显示模式
    let window_state = lnk_info.as_ref().and_then(|li| li.window_state.clone());

    // 备注：优先配置项，如有模板则使用模板
    let comment: Option<String> = lnk_info.as_ref()
        .and_then(|li| li.comment.clone())
        .or_else(|| template.and_then(|t| t.comment.as_ref().map(|tmpl| process_template(program_path, tmpl))));
    ...
    if current_shortcut.exists() {
        ...
        if let Ok(original_path) = get_shortcut_target(&current_shortcut) {
            if original_path.to_string_lossy().to_ascii_lowercase() != program_path.to_string_lossy().to_ascii_lowercase() {
                ...
                for n in 2..1000 {
                    ...
                    if path.exists() {
                        ...
                        if let Ok(existing_target) = get_shortcut_target(&path) {
                            if existing_target.to_string_lossy().to_ascii_lowercase() == program_path.to_string_lossy().to_ascii_lowercase() {
                                name = cand.clone();
                                break;
                            }
                        }
    ...
    create_shortcut(program_path, &dest.join(format!("{}.lnk", name)), args, icon, work_dir, window_state, comment)?;
