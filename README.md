...
- 🤖 **XGBoost Assisted Selection**: Ranking candidate main programs by probability, improving the accuracy of identifying complex software directories at high confidence levels.
...
### XGBoost Main Program Selection

For portable software containing multiple executables, `AutoShortcut` first eliminates unsuitable candidates such as uninstallers, installers, updaters, services, and auxiliary tools. Then, based on features like EXE name, directory level, startup path, GUI attributes, icon, PE metadata, signature, architecture, and file size, the XGBoost model predicts the probability of each candidate becoming the main program.

The model only uses the prediction result when the following conditions are met simultaneously:

- The probability of the first-ranked candidate reaches the model threshold;

- The probability difference between the first and second-ranked candidates reaches the model threshold.

If the confidence level is insufficient, the model is unavailable, or the candidates are incomplete, the program will automatically revert to the original multi-dimensional rule scoring; `exec` explicitly specified in the configuration file always has the highest priority. Therefore, XGBoost is a conservative enhancement of existing rules and does not prevent users from fixing the selection result through configuration.

The training phase uses XGBoost; the release program contains Rust numerical inference code generated by the model, but does not include `xgboost.dll`, ONNX Runtime, or other machine learning runtime libraries. The model only contains decision logic and metadata, not the software paths used for training. Training, manual annotation, and model generation methods can be found in [training/README.md](training/README.md).
