...
- 🤖 **XGBoost 辅助选择**：对候选主程序进行概率排序，高置信度时提升复杂软件目录的识别准确性
...
对于包含多个可执行文件的绿色软件，`AutoShortcut` 会先排除卸载程序、安装程序、更新器、服务和辅助工具等不适合作为主程序的候选项，再根据 EXE 名称、目录层级、启动路径、GUI 属性、图标、PE 元数据、签名、架构和文件大小等特征，由 XGBoost 模型预测各候选项成为主程序的概率。

模型只会在以下条件同时满足时采用预测结果：

- 第一名候选项的概率达到模型阈值；
- 第一名与第二名之间的概率差达到模型阈值。

置信度不足、模型不可用或候选项不完整时，程序会自动回退到原有的多维度规则评分；配置文件中显式指定的 `exec` 始终具有最高优先级。因此，XGBoost 是对现有规则的保守增强，不会阻止用户通过配置固定选择结果。

训练阶段使用 XGBoost；发布程序内置的是由模型生成的 Rust 数值推理代码，不携带 `xgboost.dll`、ONNX Runtime 或其他机器学习运行时库。模型仅包含决策逻辑和元数据，不包含参与训练的软件路径。训练、人工标注和模型生成方法见 [training/README.zh.md](training/README.zh.md)。
